...
def md_files(folder):
    """Markdown in a docs folder, plus its optional `_archive/` subfolder.

    Archiving a terminal document lowers what an agent has to read. It must not
    make the document disappear from the read model: something still refers to
    it, and a trace chain that ends in an id the renderer cannot resolve is
    ...
    out = [p for p in folder.iterdir() if visible(p)]
    archive = folder / "_archive"
    if archive.is_dir():
        out.extend(p for p in archive.iterdir() if visible(p))
    return sorted(out)
...
def build_index_md(data):
    """docs/INDEX.md — the read model for agents, as the HTML pages are for people.

    Its only job is to make "read every Decision to find out what is settled"
    unnecessary. One line per document, so a skill can pick the two ids it needs
    and open exactly those. A folder holding N documents costs one line each here
    instead of N whole files, and that gap only widens with time.

    Same guarantees as the HTML: derived, deterministic, never a source of truth.

    **Archived documents are counted, not listed, and that is a trade.** An id
    under `_archive/` can no longer be found by reading this file, so the
    invariant "every id in the tree appears in INDEX" holds only for live
    documents. It is paid knowingly: archiving exists to take a document out of
    the working set, and a read model that never shrinks when you tidy is the
    thing this file was written to avoid. Each section therefore names its
    archive folder — one bounded, predictable place to glob — rather than going
    silent, because a list that just gets shorter reads as "nothing there"
    instead of "there, elsewhere".
    """
    out = ["<!-- GENERATED by docs-kit docs-render — do not edit; "
           "regenerate with /docs-kit:docs-render -->",
           "",
           "# INDEX",
           "",
           "Một dòng mỗi tài liệu dưới `docs/`. **Đọc file này trước, rồi chỉ mở đúng id",
           "cần** — glob cả một thư mục là cách đọc đắt nhất, và nó chỉ đắt thêm theo thời",
           "gian. Nguồn sự thật vẫn là markdown; file này sinh lại mỗi lần render.",
           ""]

    def cell(v, n=90):
        s = " ".join(as_str(v).split()).replace("|", "/")
        return trim(s, n) if s else "-"

    def where(doc):
        return ("_archive/" if doc["archived"] else "") + doc["path"].name

    def section(folder, cols, rows, fmt, by_id=True):
        rows = sorted(rows, key=(lambda d: (fm_str(d, "id"), d["path"].name)) if by_id
                      else (lambda d: d["path"].name))
        # Archived documents are COUNTED here, not listed. Archiving is the kit's
        # own tidying operation, and until now it bought the reader nothing:
        # measured on a fresh scaffold, moving one Issue into `_archive/` left
        # INDEX.md at exactly 65 lines — the row simply gained a path prefix. A
        # read model that never shrinks is the thing INDEX.md exists to prevent,
        # and repos have been writing their own trimmer to get it.
        #
        # The count stays visible on purpose. A list that silently gets shorter
        # reads as "there is nothing there"; this one has to read as "there is,
        # elsewhere" — so the folder is named even when every row is gone.
        live = [r for r in rows if not r["archived"]]
        archived = len(rows) - len(live)
        head = "## %s — %d" % (folder, len(live))
        if archived:
            head += " (+%d archived)" % archived
        out.extend([head, ""])
        if live:
            out.append("```")
            out.append("# " + " | ".join(cols))
            for d in live:
                out.append(" | ".join(fmt(d)))
            out.extend(["```", ""])
        elif not archived:
            out.extend(["_trống._", ""])
        if archived:
            out.extend(["_%d đã lưu trữ, không liệt kê ở đây — xem `%s/_archive/`._"
                        % (archived, folder), ""])

    counts = " · ".join("%s %d" % (folder, len(data[k])) for folder, k in
                        (("20_issues", "issues"), ("21_proposals", "proposals"),
                         ("22_decisions", "decisions"), ("23_backlog", "backlog")))
    total_archived = sum(1 for k in data for d in data[k] if d["archived"])
    out.extend(["Layer 2: %s%s." % (counts,
                                    " · %d archived" % total_archived if total_archived else ""),
                ""])

    section("01_products", ["file", "name", "problem"], data["products"],
            lambda d: [where(d), cell(fm_str(d, "name") or doc_title(d), 40),
                       cell(fm_str(d, "problem"), 70)], by_id=False)

    def arch_row(d):
        n = len(parse_components(d["fm"].get("components", [])))
        return [where(d), "%d components" % n,
                cell(fm_str(d, "verified_at"), 20), cell(doc_title(d), 50)]

    section("02_architecture", ["file", "components", "verified_at", "title"],
            data["architecture"], arch_row, by_id=False)
    ...
    section("20_issues", ["id", "status", "lane", "file", "description"], data["issues"],
            lambda d: [cell(fm_str(d, "id"), 20), cell(fm_str(d, "status"), 12),
                       cell(fm_str(d, "lane"), 6), where(d), cell(fm_str(d, "description"))])

    section("21_proposals", ["id", "issue_ref", "file", "proposed"], data["proposals"],
            lambda d: [cell(fm_str(d, "id"), 20), cell(fm_str(d, "issue_ref"), 20),
                       where(d), cell(fm_str(d, "proposed"))])

    section("22_decisions", ["id", "outcome", "proposal_ref", "file", "reason"],
            data["decisions"],
            lambda d: [cell(fm_str(d, "id"), 20), cell(fm_str(d, "outcome"), 10),
                       cell(fm_str(d, "proposal_ref"), 20), where(d),
                       cell(fm_str(d, "reason"))])

    section("23_backlog", ["id", "status", "source_ref", "file", "description"],
            data["backlog"],
            lambda d: [cell(fm_str(d, "id"), 20), cell(fm_str(d, "status"), 12),
                       cell(fm_str(d, "source_ref"), 20), where(d),
                       cell(fm_str(d, "description"))])
...
def anchor_claims(doc):
    """Every path this document claims, with what in the document claims it.

    Mirrors `anchor_paths()` in docs_validate.sh one for one — same three sources,
    same comma split, same "a bare ```<type> line opens a fence" rule. Two
    implementations exist because the validator parses with awk and the renderer
    with Python; they must be changed together, and the samples are what catches it
    when they are not.
    """
    out = []
    for name, c in parse_components(doc["fm"].get("components", [])).items():
        if c["code"]:
            out.append((c["code"], "component:" + name))
    gen = fm_str(doc, "generated_from")
    if gen:
        # The artifact an API contract defers its volatile half to is an anchor
        # like any other: if it moves, the contract can no longer be checked.
        out.append((gen, "generated_from"))
    blocks, _ = extract_figures(doc["body"])
    for kind in blocks:
        for i, src in enumerate(blocks[kind], 1):
            for line in src.split("\n"):
                ...
                for part in v.split(","):
                    part = part.strip()
                    if part:
                        out.append((part, "%s:%d" % (kind, i)))
    return out


def build_map_tsv(data):
    """docs/MAP.tsv — the reverse index: which document claims which path.

    `INDEX.md` answers "what documents exist". This answers the question nothing
    could ask before: **who describes this file**. The relation was always in the
    markdown — a component names its `path/in/repo`, a figure fence takes a `code:`
    header — but it only ever existed inside one validator run and vanished when the
    process exited, so a hook firing on an edit had no way to consult it and fell
    back to guessing from path shape.

    Derived, deterministic, never a source of truth — the same contract INDEX.md
    holds, and `--check` compares it the same way.

    It deliberately carries **no staleness verdict**. Whether a claim has gone out of
    date is a git question whose answer changes with every commit, while this file
    changes only when the markdown does; baking the verdict in would make it stale
    within a minute of being written. `verified_at` is carried instead, so a reader
    can run the git comparison itself.
    """
    rows = []
    for key, folder in (("products", "01_products"),
                        ("architecture", "02_architecture"),
                        ("logic", "03_business-logic"),
                        ("api", "04_api")):
        for doc in data[key]:
            where = folder + "/" + ("_archive/" if doc["archived"] else "") + doc["path"].name
            rev = fm_str(doc, "verified_at") or "-"
            for path, claim in anchor_claims(doc):
                # A template's own "<file to read>" marker is not a claim about
                # anything, and a fresh scaffold should map to nothing.
                if "<" in path or ">" in path:
                    continue
                rows.append((path, where, claim, rev))

    out = ["# GENERATED by docs-kit docs-render — do not edit; "
           "regenerate with /docs-kit:docs-render",
           "# path\tdoc\tclaim\tverified_at",
           "# Một dòng mỗi lần một tài liệu layer 1 nhận mô tả một đường dẫn. `path` có thể",
           "# là glob hoặc một mục trong danh sách `code:` (STANDARD §7). Không có cột nào",
           "# nói tài liệu đã cũ hay chưa: đó là câu hỏi của git, hãy so `verified_at` với",
           "# `git diff --name-only`."]
...
def main():
    ...
    if check_only:
        ...
        # Both read models get the same gate. A stale MAP.tsv is worse than a stale
        # INDEX.md, because the Stop hook stays quiet on what the map does not list —
        # a map that has fallen behind the docs silences warnings instead of
        # producing wrong ones, and silence is the failure nobody notices.
        models = (("INDEX.md", build_index_md(loaded),
                   "skills that read it will fall back to globbing whole folders",
                   "an agent reading it would miss or misread a document"),
                  ("MAP.tsv", build_map_tsv(loaded),
                   "the Stop hook cannot tell which document describes an edited file",
                   "edits to a documented path would go unreported"))
        for name, want, missing_cost, stale_cost in models:
            target = docs / name
            if not target.is_file():
                print("docs-render --check: docs/%s is missing — %s. Run docs_render.sh."
                      % (name, missing_cost), file=sys.stderr)
                return 1
            if target.read_text(encoding="utf-8", errors="replace") != want:
                print("docs-render --check: docs/%s is stale — it no longer matches the "
                      "markdown under docs/. %s. Run docs_render.sh and commit the "
                      "result." % (name, stale_cost[0].upper() + stale_cost[1:]),
                      file=sys.stderr)
                return 1
        print("INDEX OK — docs/INDEX.md and docs/MAP.tsv match the markdown under docs/")
        return 0
